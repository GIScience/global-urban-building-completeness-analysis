            ...
            try:
                with psycopg2.connect(dns) as con:
                    with con.cursor() as cur:
                        cur.execute(query.format(table_name = "microsoft_buildings_coverage_simple"), (geom,))
                        res = cur.fetchone()
                        if res:
...
if __name__ == "__main__":
        ...
        for i in grid_df.index:
            ...
            for try_no in range(4):
                try:
                    with psycopg2.connect(dns) as con:
                        with con.cursor() as cur:
                            cur.execute(query.format(table_name = "microsoft_buildings"), (geom,))
                            res = cur.fetchone()
                            if res is None or res[0] is None:
                                res=0
                            else:
                                res = res[0]/(1000*1000)
                            grid_df.at[i, "microsoft_area"] = res
                except Exception as e:
                    if try_no < 4-1:
                        logging.warning(
                            f"An error occured: UC ID {urban_center_id}, Index {i}. Errormessage: {e}"
                        )
                        # wait 30 seconds before retrying
                        time.sleep(30)
                        continue
    ...
    uc_ms_building_area = pd.concat(collected_table)
    uc_ms_building_area.to_csv("uc_ms_building_area.csv")